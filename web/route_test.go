...
func Test_router_AddRoute(t *testing.T) {
	...
	mockHandler := func(c *context.Context) {}
	r := newRouter()
...
func Test_findRoute_Middleware(t *testing.T) {
	var mdlBuilder = func(i byte) middleware.Middleware {
	...
	mdlsRoute := []struct {
	...
	}{
		{
			method: http.MethodGet,
			path:   "/a/b",
			mdls:   []middleware.Middleware{mdlBuilder('a'), mdlBuilder('b')},
		},
		{
			method: http.MethodGet,
			path:   "/a/*",
			mdls:   []middleware.Middleware{mdlBuilder('a'), mdlBuilder('*')},
		},
		{
			method: http.MethodGet,
			path:   "/a/b/*",
			mdls:   []middleware.Middleware{mdlBuilder('a'), mdlBuilder('b'), mdlBuilder('*')},
		},
		{
			method: http.MethodGet,
			path:   "/a/:id(^[0-9]+$)/c",
			mdls:   []middleware.Middleware{mdlBuilder('a'), mdlBuilder('r'), mdlBuilder('c')},
		},
		{
			method: http.MethodPost,
			path:   "/a/b/*",
		...
		},
		{
			method: http.MethodPost,
			path:   "/a/*/c",
			mdls:   []middleware.Middleware{mdlBuilder('a'), mdlBuilder('*'), mdlBuilder('c')},
		},
		{
			method: http.MethodPost,
			path:   "/a/b/c",
			mdls:   []middleware.Middleware{mdlBuilder('a'), mdlBuilder('b'), mdlBuilder('c')},
		},
		{ // 注意， 会匹配全部
			method: http.MethodDelete,
			path:   "/*",
			mdls:   []middleware.Middleware{mdlBuilder('*')},
		},
		{
			method: http.MethodDelete,
			path:   "/",
